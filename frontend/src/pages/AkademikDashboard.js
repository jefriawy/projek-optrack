...
import { FaUser, FaCheckCircle, FaSpinner, FaClock, FaCommentDots } from 'react-icons/fa';
...
const AkademikDashboard = () => {
  ...
  const summaryStats = useMemo(() => {
    ...
    const delivered = trainingData.filter(t => t.statOpti === 'Delivered').length;
    const onProgress = trainingData.filter(t => t.statOpti === 'On Progress').length;
    const received = trainingData.filter(t => t.statOpti === 'Received').length;
  ...
  return (
    <div className="p-8 bg-gray-100 min-h-screen">
      <h1 className="text-3xl font-bold text-gray-800 mb-6">Akademik Dashboard</h1>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        <div className="bg-white p-6 rounded-xl shadow-md flex items-center justify-between">
          <div>
            ...
            <p className="mt-1 text-3xl font-bold text-gray-900">{summaryStats.total}</p>
          </div>
          <IoIosPeople className="text-5xl text-blue-500" />
        </div>
        <div className="bg-white p-6 rounded-xl shadow-md flex items-center justify-between">
          <div>
            <p className="text-sm font-medium text-gray-500">Training Selesai</p>
            <p className="mt-1 text-3xl font-bold text-gray-900">{summaryStats.delivered}</p>
          </div>
          <FaCheckCircle className="text-5xl text-green-500" />
        </div>
        <div className="bg-white p-6 rounded-xl shadow-md flex items-center justify-between">
          <div>
            <p className="text-sm font-medium text-gray-500">Training Berjalan</p>
            <p className="mt-1 text-3xl font-bold text-gray-900">{summaryStats.onProgress}</p>
          </div>
          <FaSpinner className="text-5xl text-yellow-500" />
        </div>
        <div className="bg-white p-6 rounded-xl shadow-md flex items-center justify-between">
          <div>
            <p className="text-sm font-medium text-gray-500">Training Mendatang</p>
            <p className="mt-1 text-3xl font-bold text-gray-900">{summaryStats.received}</p>
          </div>
      ...
      <div className="bg-white p-6 rounded-xl shadow-md mb-8">
        <h2 className="text-2xl font-semibold text-gray-800 mb-4">Daftar Semua Training</h2>
