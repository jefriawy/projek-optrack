...
const createOpti = async (req, res) => {
  ...
  try {
    ...
    const optiData = {
      ...
      idExpert:
        b.jenisOpti === "Training" || b.jenisOpti === "Project"
          ? toNull(b.idExpert) ? Number(b.idExpert) : null
          : null,
      idHR:
        b.jenisOpti === "Outsource"
          ? toNull(b.idExpert) ? Number(b.idExpert) : null
          : null,
      idPM: toNull(b.idPM) ? Number(b.idPM) : null,
      valOpti:
...
const updateOpti = async (req, res) => {
  ...
  try {
    ...
    const optiData = {
      ...
      idHR:
        b.jenisOpti === "Outsource"
          ? toNull(b.idExpert) ? Number(b.idExpert) : existingOpti.idHR
          : null,
      idPM: toNull(b.idPM) ? Number(b.idPM) : existingOpti.idPM,
      valOpti:
    ...
    if (isStatusChangedToPoReceived) {
      const createRelatedEntity = async (type) => {
        const idOpti = id;
        if (type === "Outsource") {
          // Cek apakah sudah ada outsource untuk opti ini
          const [existing] = await connection.query("SELECT idOutsource FROM outsource WHERE idOpti = ? LIMIT 1", [idOpti]);
          if (existing.length === 0) {
            const newId = await generateUserId("Outsource");
            await connection.query(
              `INSERT INTO outsource (idOutsource, nmOutsource, idOpti, idCustomer, startOutsource, endOutsource) VALUES (?, ?, ?, ?, ?, ?)`,
              [
                newId,
                optiData.nmOpti,
                idOpti,
      ...
      } else if (optiData.jenisOpti === "Outsource") {
        await createRelatedEntity("Outsource");
      }
    }
    ...
    // Sinkronisasi data ke tabel outsource saat Opti Outsource di-update
    else if (
      optiData.jenisOpti === "Outsource" &&
      (existingOpti.statOpti === "po received" ||
        existingOpti.statOpti === "opti on going")
    ) {
      const [outsourcesToUpdate] = await connection.query(
        "SELECT idOutsource FROM outsource WHERE idOpti = ?",
        [id]
      );
      if (outsourcesToUpdate.length > 0) {
        const outsourceId = outsourcesToUpdate[0].idOutsource;
        await connection.query(
          `UPDATE outsource SET
             nmOutsource = ?, startOutsource = ?, endOutsource = ?, idCustomer = ?
           WHERE idOutsource = ?`,
          [
            optiData.nmOpti,
            optiData.startProgram,
